...
const run = async () => {
  ...
  const postFeedResponse = await axios.post(feedEndpoint, {
    ...
    excludedHashtags: ['#not-astronomy', '#not-astrophysics'],
    mentions: ['@user1', '@user2'],
    excludedMentions: ['@user3', '@user4'],
    search: ['nebula', 'galaxy', 'star'],
    excludedSearch: ['not-nebula', 'not-galaxy', 'not-star'],
  ...
  await axios.put(`${feedEndpoint}/${feedIdentifier}`, {
    ...
    excludedHashtags: ['#not-astronomy-modified', '#not-astrophysics-modified'],
    mentions: ['@user5', '@user6'],
    excludedMentions: ['@user7', '@user8'],
    search: ['nebula-modified', 'galaxy-modified', 'star-modified'],
    excludedSearch: ['not-nebula-modified', 'not-galaxy-modified', 'not-star-modified'],
    includedAtUris: ['at://did:plc:.../app.bsky.feed.post/...'],
    excludedAtUris: ['at://did:plc:.../app.bsky.feed.post/...'],
    operator: 'OR',
  })
